...
### 6. 느낀점
개발공부를 시작한 지 6개월 만에 프로젝트를 수행하면서, 개발이라는 행위가 `추상적인 것을 구현한다는 점에서 구성원 개개인이 상상하는 구현체가 다를 수 있다는 점`을 많이 느꼈고, 공통된 구현체를 머리속에 품기 위해, 9시, 18시 회의를 통해, 애매한 개념을 맞추려고 노력했고, 실무에서도 고민을 나눠야겠다는 생각을 하게 되었습니다.   
